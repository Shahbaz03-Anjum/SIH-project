...
import { trainingRecommendations } from '../../lib/mockData';
...
const categoryOptions = ['All categories', 'Cloud', 'Data', 'Frontend', 'Database'];
...
const demandRows: DemandRow[] = [
  { skill: 'AWS', industryDemand: 82, studentProficiency: 42, studentsAffected: 68, growth: 18 },
  { skill: 'React', industryDemand: 76, studentProficiency: 68, studentsAffected: 54, growth: 14 },
  { skill: 'Python', industryDemand: 88, studentProficiency: 82, studentsAffected: 31, growth: 11 },
  { skill: 'SQL', industryDemand: 71, studentProficiency: 76, studentsAffected: 22, growth: 9 },
  { skill: 'Docker', industryDemand: 63, studentProficiency: 51, studentsAffected: 59, growth: 22 },
];
...
export default function IndustryDemandPage() {
  ...
  const filteredRows = useMemo(() => {
    return demandRows.filter((row) => {
      const categoryMatch = category === 'All categories' || trainingRecommendations.some((recommendation) => recommendation.skill === row.skill && recommendation.category === category);
      return categoryMatch;
    });
  }, [category]);

  const mostDemandedSkill = filteredRows.reduce((top, current) => current.industryDemand > top.industryDemand ? current : top, filteredRows[0]);
  const emergingSkill = [...filteredRows].sort((a, b) => b.growth - a.growth)[0];
  const avgDemand = Math.round(filteredRows.reduce((sum, row) => sum + row.industryDemand, 0) / filteredRows.length);
  const criticalGap = filteredRows.reduce((top, current) => current.industryDemand - current.studentProficiency > top.industryDemand - top.studentProficiency ? current : top, filteredRows[0]);
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
          <div className="bg-white p-5 rounded-lg shadow-sm">
            ...
            <div className="mt-1 text-xs text-emerald-600">{mostDemandedSkill.industryDemand}% demand intensity</div>
          </div>
          <div className="bg-white p-5 rounded-lg shadow-sm">
            <div className="text-sm text-slate-500">Emerging Skill</div>
            <div className="mt-2 text-2xl font-bold">{emergingSkill.skill}</div>
            <div className="mt-1 text-xs text-emerald-600">+{emergingSkill.growth}% growth</div>
          </div>
          <div className="bg-white p-5 rounded-lg shadow-sm">
          ...
          <div className="bg-white p-5 rounded-lg shadow-sm">
            ...
            <div className="mt-2 text-2xl font-bold">{criticalGap.skill}</div>
            <div className="mt-1 text-xs text-rose-600">{criticalGap.industryDemand - criticalGap.studentProficiency} point mismatch</div>
          </div>
        </div>
              ...
              {filteredRows.map((row) => (
                <div key={row.skill}>
                  <div className="mb-1 flex items-center justify-between text-sm">
                    ...
                    <span className="text-slate-500">Demand {row.industryDemand}% / Proficiency {row.studentProficiency}%</span>
                  </div>
                  <div className="h-2.5 rounded bg-slate-100">
                    <div className="h-2.5 rounded bg-sky-500" style={{ width: `${row.industryDemand}%` }} />
                  </div>
                  <div className="mt-1 flex justify-between text-[11px] text-slate-500">
                    <span>Students affected: {row.studentsAffected}</span>
                ...
                .map((row) => (
                  <div key={row.skill} className="flex items-center justify-between rounded border border-slate-200 p-3">
                    <div>
                      ...
                      <div className="text-xs text-slate-500">{row.industryDemand}% demand</div>
                    </div>
              ...
              {filteredRows.map((row) => (
                <div key={row.skill} className="rounded border border-slate-200 p-3">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-sm text-slate-500">{row.industryDemand}% demand</div>
                  </div>
              ...
              {filteredRows.map((row) => (
                <div key={row.skill} className="rounded border border-slate-200 p-3">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <div className="font-medium">{row.skill}</div>
                      <div className="text-xs text-slate-500">Industry Demand: {row.industryDemand}% | Student Proficiency: {row.studentProficiency}%</div>
                    </div>
                    <div className="text-sm text-rose-600">{row.studentsAffected} students affected</div>
                  </div>
                  <div className="mt-3 flex gap-2">
                    <button className="rounded bg-sky-600 px-3 py-1.5 text-xs text-white hover:bg-sky-700">View Students</button>
                    ...
                    <button className="rounded bg-emerald-100 px-3 py-1.5 text-xs text-emerald-700 hover:bg-emerald-200">Create Assessment</button>
                  </div>
                </div>
              ))}
            </div>
